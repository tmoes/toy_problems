 ...
 */
 ...
 var Range = function(start, end, step) {
  // If end is not provided, defaults to start value
  end === undefined ? this.end = start : this.end = end;

  // If start < end step = 1, else step = -1, else step = step
  if (step === undefined) {
    start < end ? this.step = 1 : this.step = -1;
  } else {
    this.step = step;
  }
